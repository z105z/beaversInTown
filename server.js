...
io.on('connection', function(socket) {
    ...
    socket.on('disconnect', function() {
        ...
        if(players[socket.id]){
            connection.query("UPDATE `coordtable` SET  `coordY` = '" + players[socket.id].y + "' WHERE  `coordtable`.`id` =  '" + players[socket.id].iddb + "'" , function(err, result){});
            connection.query("UPDATE `coordtable` SET  `coordX` = '" + players[socket.id].x + "' WHERE  `coordtable`.`id` =  '" + players[socket.id].iddb + "'" , function(err, result){});
            connection.query("UPDATE `coordtable` SET  `status` = 'offline' WHERE `coordtable`.`id` =  '" + players[socket.id].iddb + "'" , function(err, result){});
        }
    ...
    socket.on('authorization', function(login, password){
         connection.query("SELECT * FROM coordtable WHERE login = '" + login + "' AND password = '" + password + "'", function(err, result){
            if(result!=0&&result[0].status == 'offline'){
                ...
                y = result[0].coordY;
                iddb = result[0].id;
                connection.query("UPDATE `coordtable` SET  `status` = 'online' WHERE `coordtable`.`id` =  '" + iddb + "'" , function(err, result){});
                connection.query("SELECT * FROM coordsgame", function(err, result){
                    if(result!=0){
                        console.log(result);
